...
class ActiveField extends \yii\widgets\ActiveField
{
     ...
     * @var string the template for checkboxes in default layout
     */
    public $switchTemplate = "<div class=\"togglebutton\">\n{beginLabel}\n{input}\n{labelTitle}\n{endLabel}\n{error}\n{hint}\n</div>";
    /**
     ...
     * @var string the template for checkboxes in horizontal layout
     */
    public $horizontalSwitchTemplate = "{beginWrapper}\n<div class=\"togglebutton\">\n{beginLabel}\n{input}\n{labelTitle}\n{endLabel}\n</div>\n{error}\n{endWrapper}\n{hint}";
    /**
    ...
    public function editor($options = [])
    {
        ...
        $editor = Html::tag("div", Html::getAttributeValue($this->model, $this->attribute), ["id" => "summernote"]);
        $this->parts["{label}"] = "";
        ...
        $this->form->view->registerJs('
          ...
          function resizeNoteEditorPostion() {
            var editorBottomPos = $(".note-editor").offset().top + $(".note-editor").height(),
                windowHeight = $(window.document).height()
            if($("footer.content-footer").length != 0 && windowHeight - editorBottomPos < 100) {
              $(".note-editor").css("margin-bottom", "200px")
            }
          }
          // 开启编辑器
     ...
     */
     public function switch($options = [], $enclosedByLabel = true)
     {
         if ($enclosedByLabel) {
             if (!isset($options['template'])) {
                 $this->template = $this->form->layout === 'horizontal' ?
                     $this->horizontalSwitchTemplate : $this->switchTemplate;
             } else {
                 $this->template = $options['template'];
                 unset($options['template']);
             }
