...
class ActiveField extends \yii\widgets\ActiveField {
	...
	public function editorInput($options = []) {
		...
		$this->form->view->registerJs('
			var quill_input = jQuery("[name=\'' . $name . '\']"),
				lastcursor = 0
            ...
            quill.on("text-change", function(delta, source) {
                ...
                if (range) {
                   lastcursor = range.index
               	} else {
               		lastcursor = 0
               	}
                quill_input.val(jQuery("#quill-editor .ql-editor").html())
            });
			...
			quill.pasteHTML(0, quill_input.val());

			if($("#quill-toolbar").parents("#dashboard-content").length == 1) {
				$("#dashboard-content").scroll(function() {
					var content_top = $("#dashboard-content").scrollTop()
	                if(content_top >= 72) {
	                	$("#quill-toolbar").css({
	                		position: "fixed",
	                		"z-index": 1000,
	                		"width": $("#quill-editor").width(),
	                		top: 72
	                	})
	                } else {
	                	$("#quill-toolbar").removeAttr("style")
	                }
				})

				$(document).one("unload.dashboard", "#dashboard-content", function() {
					$("#dashboard-content").off("scroll")
				})
			} else {
				$("#dashboard-modal").scroll(function() {
                    if($("#quill-toolbar").offset().top <= 0) {
                        $("#quill-toolbar").css({
                            position: "fixed",
                            "z-index": 1000,
                            "width": $("#quill-editor").width(),
                            top: $("#dashboard-modal").scrollTop() - 40
                        })
                    } else {
                        $("#quill-toolbar").removeAttr("style")
                    }
				...
				$(document).one("unload.dashboard", "#dashboard-modal", function() {
					$("#dashboard-modal").off("scroll")
				})
			}
	...
	// console.log(jQuery("#quill-editor .ql-editor").html().replace(/\s/, "&nbsp;"))
