...
<?php if ($generator->enableUpload): ?>
     ...
     */
     public function scenarios() {
         return [
             'default' => [<?php foreach ($labels as $name => $label) { echo "'$name', "; } ?> ],
             'upload' => ['file']
         ];
     }
<?php endif; ?>
...
<?php if ($generator->enableUpload): ?>
    public function upload() {
        ...
        $filepath = $this->path . "/";
        if ($this->validate()) {
            $filename = hash_file('md5', $this->file->tempName) . '.' . $this->file->extension;
            ...
            try {
                ...
                if($filepath != $this->path && file_exists($filepath)) {
                    unlink($filepath);
                }
                return false;
            }
